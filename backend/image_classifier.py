...
class MaterialClassifier:
    ...
    def extract_features(self, image: np.ndarray) -> np.ndarray:
        ...
        features.extend([
            ...
            np.mean(edges[edges > 0]) if np.any(edges > 0) else 0,  # Average edge strength
        ])
        ...
        if self.is_trained and self.model is not None:
            ...
            try:
                ...
                return {
                    ...
                    "method": "ml_model",
                    "probabilities": {
                        "organic": float(probabilities[1] if len(probabilities) > 1 else 0.5),
                        "non_organic": float(probabilities[0] if len(probabilities) > 0 else 0.5)
                    }
                }
    ...
    def _rule_based_classify(self, image: np.ndarray, features: np.ndarray) -> Dict[str, any]:
        """
        ...
        return {
            ...
            "scores": {
                ...
                "non_organic": non_organic_score
            }
        }
    
    def train_model(self, training_data: list, labels: list, save_path: str = "models/material_classifier.pkl"):
        """
